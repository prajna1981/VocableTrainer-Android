...
public class VEntryEditorDialog extends DialogFragment {
    ...
    private void generateMeanings(List<String> meanings, String hint, LinearLayout layout) {
        final View.OnClickListener addListener = new View.OnClickListener() {
            ...
            public void onClick(View v) {
                layout.addView(generateMeaning("", hint,IMG_ADD,this,true));
        ...
        if(meanings.size() > 0) {
            for (int i = 0; i < meanings.size() - 1; i++) {
                layout.addView(generateMeaning(meanings.get(i), hint, IMG_REMOVE, new DeleteAction(tagCounter,layout),false));
            }
            layout.addView(generateMeaning(meanings.get(meanings.size() - 1), hint, IMG_ADD, addListener,false));
        }else{
            layout.addView(generateMeaning("", hint, IMG_ADD, addListener,true));
        }
    }
     ...
     * @return View
     */
    private View generateMeaning(final String meaning, final String hint,int image , View.OnClickListener listener,
                                 boolean focus){
        View view = View.inflate(getActivity(),R.layout.meaning,null);
